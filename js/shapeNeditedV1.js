...
Shape.prototype.writeLecter = function () {
    ...
    if (this.position === "center") {
        var isWord=JsonObj.exercises[10].isWord;
        //for (var k = 0; k < JsonObj.length; k++) {
           ...
           // if (JsonObj[k].Title === this.lecter) {
                ...
                for(var index=0;index<symbols.length;index++){
                  //offset=maxYWord-(maxY[index]-minY[index]);
                  offeset=0;
                  if(index>0){
                  ...
                  for (var i = 0; i < strokes.length; i++) {
                    ...
                    for (var j = 0; j < points.length ; j++) {
                        if (j === 0){
                          this.bmd.context.moveTo(points[j].X - minX[index] +lastSpace+this.bitmapW/2-lenW/2, points[j].Y - minY[index]+this.bitmapH/2-maxYWord/2+offset);  
                        }else{
                          this.bmd.context.lineTo(points[j].X - minX[index] +lastSpace+this.bitmapW/2-lenW/2, points[j].Y - minY[index]+this.bitmapH/2-maxYWord/2+offset);
                        }
                    }
                }
